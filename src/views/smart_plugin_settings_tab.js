...
export class SmartPluginSettingsTab extends PluginSettingTab {
  constructor(app, plugin) {
    ...
    const is_pro = this.env?.is_pro ?? true;
    if(is_pro && !this.env_settings_tab) this.plugin.addSettingTab(new SmartEnvSettingTab(this.plugin.app, this.plugin));
  ...
  prepare_layout() {
    ...
    this.containerEl.empty();
    this.header_container = this.containerEl.createDiv({
      cls: "smart-plugin-settings-header",
    });
    this.plugin_container = this.containerEl.createDiv({
      cls: "smart-plugin-settings-main",
    });
    this.global_settings_container = this.containerEl.createDiv({
      cls: "smart-plugin-settings-env",
    });
    this.pro_plugins_container = this.containerEl.createDiv({
      cls: "smart-plugin-settings-pro-plugins",
    });
  }
  ...
  async render_global_settings(container) {
    ...
    if(is_pro) {
      ...
      const control_div = settings_item_div.createDiv({
      ...
      const settings_smart_env = await this.render_component(
        "settings_smart_env",
        this.env,
      );
      if (settings_smart_env) container.appendChild(settings_smart_env);
    }
    const smart_plugins_settings = await this.render_component(
      "pro_plugins_list",
      this.env,
    );
  ...
  async render_component(name, scope, params = {}) {
    return await this.env.smart_components.render_component(
      name,
      scope,
      params,
    );
...
export class SmartEnvSettingTab extends PluginSettingTab {
  ...
  async render() {
    ...
    this.header_container.createEl("p", {
      text: "Manage all global Smart Environment settings from one tab. These settings apply to all Smart Plugins.",
    });
    const settings_smart_env = await this.render_component(
      "settings_smart_env",
      this.env,
    );
    if (settings_smart_env)
      this.plugin_container.appendChild(settings_smart_env);
    const smart_plugins_settings = await this.render_component(
      "pro_plugins_list",
      this.env,
    );
  ...
  if (env.state !== "loaded") {
    if (env.state === "loading") {
      ...
      container.createEl("p", {
        text: "Smart Environment not yet initialized.",
      });
      const load_btn = container.createEl("button", {
        text: "Load Smart Environment",
      });
