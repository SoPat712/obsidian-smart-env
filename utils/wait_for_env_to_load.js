  ...
  if (!wait_for_states.includes(scope.env?.state)) {
    ...
    while (scope.env.state === 'init' && Platform.isMobile && !clicked_load_env) {
      if(container){
        // button to load env
        container.empty();
        scope.env.smart_view.safe_inner_html(
          container,
          "<button>Load Smart Environment</button>",
        );
    ...
    while (!wait_for_states.includes(scope.env.state)) {
      if (container) {
        const loading_msg = scope.env?.obsidian_is_syncing
          ? "Waiting for Obsidian Sync to finish..."
          : "Loading Obsidian Smart Environment...";
        container.empty();
        // set loading message
